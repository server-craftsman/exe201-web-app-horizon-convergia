      ...
      try {
        ...
        if (data) {
          ...
          const categoriesWithChildren: CategoryWithChildren[] = parentCategories.map((parent) => ({
            ...parent,
            children: allCategories.filter((child) => child.parentCategoryId === parent.id),
          }));
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 h-full flex items-center">
        <div className="w-1/4 bg-white/90 backdrop-blur-sm p-4 rounded-lg shadow-lg h-[calc(100%-4rem)] overflow-y-auto">
          <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">Danh mục sản phẩm</h3>
          {categoriesLoading ? (
              ...
              {categories.map((parent) => (
                ...
                >
                  <Link
                    ...
                    className="flex items-center justify-between w-full px-3 py-2 text-sm text-gray-700 hover:bg-amber-100 rounded-md transition-colors duration-200"
                  >
                  ...
                  {parent.children.length > 0 && activeParentCategory === parent.id && (
                    <div className="absolute left-full top-0 w-64 bg-white shadow-2xl rounded-r-lg p-2 z-20">
                      <ul>
                        {parent.children.map((child) => (
                          <li key={child.id}>
                            <Link
                              to={`${ROUTER_URL.CLIENT.PRODUCT_LIST_BY_CATEGORY_ID}/${child.id}`}
                              className="block px-3 py-2 text-sm text-gray-700 hover:bg-amber-100 rounded-md"
                            >
                              {child.name}
                            </Link>
                          </li>
